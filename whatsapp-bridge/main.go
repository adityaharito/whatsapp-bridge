...
func sendWhatsAppMessage(client *whatsmeow.Client, recipient string, message string, mediaPath string) (bool, string) {
	...
	if mediaPath != "" {
		...
		// Determine media type using multiple methods
		var mediaType whatsmeow.MediaType
		var mimeType string

		// Method 1: Try file extension
		...
		// Determine media type based on both extension and content detection
		switch {
...
// detectFileExtension tries to detect file type from magic bytes
func detectFileExtension(filePath string) string {
...
// downloadFromURL downloads a file from a URL and saves it to a temporary location
func downloadFromURL(url string) (string, error) {
	// Create a temporary file with .tmp extension first
	tmpFile, err := os.CreateTemp("", "whatsapp_media_*.tmp")
	if err != nil {
	...
	case "image/webp":
		fileExt = "webp"
	default:
		// Try to detect from first few bytes if content type is not helpful
		fileExt = detectFileExtension(tmpFile.Name())
	}

	// Rename the file with proper extension
	finalPath := strings.TrimSuffix(tmpFile.Name(), ".tmp") + "." + fileExt
...
func startRESTServer(client *whatsmeow.Client, messageStore *MessageStore, port int) (int, error) {
	...
	mux.HandleFunc("/api/send", func(w http.ResponseWriter, r *http.Request) {
		...
		if req.Message == "" && req.MediaPath == "" && req.MediaURL == "" {
			http.Error(w, "Message, media path, or media URL is required", http.StatusBadRequest)
			return
		}

		fmt.Println("Received request to send message", req.Message, req.MediaPath, req.MediaURL)

		// Handle media URL if provided
		actualMediaPath := req.MediaPath
		var downloadedFile string

		if req.MediaURL != "" {
			// Download from URL to temp file
			tempPath, err := downloadFromURL(req.MediaURL)
			if err != nil {
				w.Header().Set("Content-Type", "application/json")
				w.WriteHeader(http.StatusInternalServerError)
				json.NewEncoder(w).Encode(SendMessageResponse{
					Success: false,
					Message: fmt.Sprintf("Failed to download media from URL: %v", err),
				})
				return
			}
			actualMediaPath = tempPath
			downloadedFile = tempPath
			fmt.Printf("Downloaded media from URL to: %s\n", tempPath)
		}

		// Send the message
		success, message := sendWhatsAppMessage(client, req.Recipient, req.Message, actualMediaPath)
